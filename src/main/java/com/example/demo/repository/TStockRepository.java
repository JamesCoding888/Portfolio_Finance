...
@Repository // e.q. @Repository(value = "tStockRepository")
public interface TStockRepository extends JpaRepository<TStock, Integer> {
	
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "UPDATE TStock SET name=?2, symbol=?3, classify_id=?4 WHERE id=?1")
    ...
    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "UPDATE TStock SET change_price=?2, change_in_percent=?3, pre_closed=?4, price=?5, transaction_date=?6, volumn=?7 WHERE id=?1")
